...
| Source-only minimal GDAL/OGR asset production | Locked local source/prefix/notice trees and CMake/Ninja/MSVC/dumpbin/Node identities; fixed profile; cache, notice, CRS, layout, and PE-closure checks; five fake-tool tests | Staging boundary proven; no original source-archive verification, actual link-input/complete-driver inventory, real toolchain/prefix recipe, notices, PE artifact, second-build comparison, release URL, or catalog promotion |
