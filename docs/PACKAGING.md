...
- FFmpeg 8.x is an optional runtime adapter for transcoded H.264/AAC incident review clips and lazy timeline hover thumbnails. Set `ROADWATCHER_FFMPEG` to `ffmpeg.exe` or place it on `PATH`. When unavailable, export still produces JSON, HTML, images, synchronized GPX excerpts, and the integrity manifest, plus `FFMPEG-SETUP.txt` with the recovery steps. A hover preview shows the same setup requirement; an existing cached thumbnail remains available offline.
